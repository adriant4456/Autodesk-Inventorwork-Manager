...
def no_proj(project, machine, folder=None):
...
class Timesheet_Entry:
    ...
    def __init__(self, date, start_time, wbs, folder):
        self.date = date
...
def log_timesheet_entry(entry :Timesheet_Entry):
    ...
    for row in sheet.iter_rows(min_row=1, min_col=2, max_col=2,max_row=sheet.max_row):
        for cell in row:
            try:
    ...
    if entry_row == (sheet.max_row + 1):
        sheet.cell(row = entry_row, column = 2).value = entry.get_wbs()
    # Also add corresponding folder name in first column
        sheet.cell(row=entry_row, column = 1).value = entry.get_folder()
...
def instantiate_timesheet_entry(network = None, activity = None) -> Timesheet_Entry:
    if network is None and activity is None:
        ...
        save_wbs_data(wbs)
    #get folder name as well
    with open('C:\\InventorWork\\folder_name.txt','r') as file:
        folder = file.read()
    date = datetime.datetime.now().date()
    start_time = time.time()
    global active_timesheet_entry
    active_timesheet_entry = Timesheet_Entry(date, start_time, wbs, folder)
    return 
...
def newclick(project, machine):
    if messagebox.askyesno('Restart', "Are you sure you want to restart Inventor and create new InventorWork?"):
        rename_load = Toplevel(root)
        rename_load.transient()
        app = loadwindow(rename_load)
        rename_load.update()
        try:
            make(project, machine)
